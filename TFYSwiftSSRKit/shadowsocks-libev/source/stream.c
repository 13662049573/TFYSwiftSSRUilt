...
stream_get_cipher_type(int method)
{
    ...
    const mbedtls_cipher_info_t *info = mbedtls_cipher_info_from_string(mbedtlsname);
    return (cipher_kt_t *)info;
}
...
stream_cipher_ctx_init(cipher_ctx_t *ctx, int method, int enc)
{
    ...
    if (ctx->evp == NULL) {
        FATAL("Cannot allocate cipher context");
    }
...
stream_key_init(int method, const char *pass, const char *key)
{
    ...
    if (method == SALSA20 || method == CHACHA20 || method == CHACHA20IETF) {
        ...
        const mbedtls_cipher_info_t *info = mbedtls_cipher_info_from_values(
            ...
            supported_stream_ciphers_key_size[method] * 8,
            MBEDTLS_MODE_CFB);
...
size_t stream_ctx_get_iv_size(const cipher_ctx_t *cipher_ctx)
{
    if (cipher_ctx == NULL || cipher_ctx->cipher == NULL) {
        return 0;
    }
    return cipher_ctx->cipher->nonce_len;
}

size_t stream_ctx_get_key_size(const cipher_ctx_t *cipher_ctx)
{
    if (cipher_ctx == NULL || cipher_ctx->cipher == NULL) {
        return 0;
    }
