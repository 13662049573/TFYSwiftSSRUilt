...
main(int argc, char **argv)
{
    ...
    for (i = 0; i < remote_num; i++) {
        ...
        if (get_sockaddr(remote_addr[i].host, remote_addr[i].port, storage, 1, ipv6first) == -1) {
    ...
    if (mode != TCP_ONLY) {
        ...
        init_udprelay(local_addr, local_port,
#ifdef MODULE_LOCAL
                     listen_ctx.remote_addr[0], sizeof(struct sockaddr_storage),
#ifdef MODULE_TUNNEL
                     tunnel_addr,
#endif
#endif
