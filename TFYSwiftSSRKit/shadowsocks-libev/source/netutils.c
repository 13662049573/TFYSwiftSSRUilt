...
#ifdef SO_NOSIGPIPE
int
set_nosigpipe(int socket)
{
    int opt = 1;
    return setsockopt(socket, SOL_SOCKET, SO_NOSIGPIPE, &opt, sizeof(opt));
}
#endif
...
{
    int opt = 1;
    return setsockopt(socket, SOL_SOCKET, SO_BROADCAST, &opt, sizeof(opt));
}
#endif
