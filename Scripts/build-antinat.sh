...
DEPS_ROOT="$PROJECT_ROOT/TFYSwiftSSRKit/antinat"
VERSION="0.93"
LIB_NAME="antinat"
...
build_antinat() {
    ...
    if [ ! -f "${LIB_NAME}.tar.gz" ]; then
        ...
        local DOWNLOAD_URL="https://downloads.sourceforge.net/project/antinat/antinat/${VERSION}/antinat-${VERSION}.tar.gz"
        if ! curl -L --retry 3 --retry-delay 2 -o "${LIB_NAME}.tar.gz" "$DOWNLOAD_URL" --insecure; then
...
create_universal_library() {
    ...
    if ! cp "$INSTALL_DIR/ios/lib/libantinat.a" "$INSTALL_DIR/lib/libantinat_ios.a"; then
    ...
    if ! xcrun lipo -create \
        ...
        -output "$INSTALL_DIR/lib/libantinat_macos.a"; then
    ...
    if [ ! -f "$INSTALL_DIR/lib/libantinat_ios.a" ] || \
       [ ! -f "$INSTALL_DIR/lib/libantinat_macos.a" ]; then
